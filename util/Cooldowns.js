...
class Cooldowns {
    ...
    async loadCooldowns() {
        ...
        console.info('Loading cooldowns from the database...')
        let i = 0
        ...
        console.info(chalk.green`Loaded ${i} cooldowns from the database.`)
    }
    ...
    verifyCooldown(duration) {
        ...
        if (split.length !== 2) {
            throw new Error(`Duration "${duration}"is an invalid duration. please use "10 m", "10 h" etc.`)
        }
        ...
        if (!cooldownDurations[type]) {
            throw new Error(`Unknown duration type "${type}". please use one of the followwing: ${Object.keys(cooldownDurations)}`)
        }
    ...
    async start({
        ...
        if (!cooldownTypes.includes(cooldownType)) {
            throw new Error(`Invalid cooldownType "${cooldownType}". Please use one of the following: ${cooldownTypes}`)
        }
...
module.exports.cancelCooldown = this.cancelCooldown